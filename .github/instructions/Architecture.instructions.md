...
## Стиль кода и защитные проверки
*   **Нет разбросанных проверок на `null`/`undefined` внутри метода.** Либо проверка не нужна (зависимость гарантированно инициализирована), либо выполняется **одна общая проверка всех требуемых зависимостей в начале метода** с выбросом информативной ошибки.
*   Предпочтительно не использовать `?.` и `!` для полей, назначаемых через инспектор: либо проверить их в начале метода, либо явно гарантировать инициализацию.